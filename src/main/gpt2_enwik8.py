...
def train_gpt2_on_enwik8(
    ...
    data_percentage = 0.1, verbose: bool = True
):
    """
...
def hyperparam_search(data_path: str, output_path: str, k: int, data_percentage = 0.1, verbose = True):
    """
        ...
        for batch_size in [2 ** i for i in range(6, 13)]:
            for lr in [10 ** j for j in range(-6, 0)]:
                for epochs in range(5, 25, 5):
                    if verbose:
                        print(f"Training GPT2 on {data_percentage*100}% Enwik8: {seq_len} seq, {batch_size} batch, {lr} learning rate, {epochs} epochs")
                    val_avg_loss, val_acc, val_topk_acc, test_avg_loss, test_acc, test_topk_acc = train_gpt2_on_enwik8(data_path, output_path, k, seq_len=seq_len, batch_size=batch_size, lr=lr, epochs=epochs, data_percentage=data_percentage)
                    df = pd.concat(df, pd.Series(['enwik8', data_percentage, seq_len, batch_size, lr, epochs, 
                                                  val_avg_loss, val_acc, val_topk_acc, test_avg_loss, test_acc, test_topk_acc]))
...
def main(args: Namespace):
    ...
    if args.mode == "train":
        train_gpt2_on_enwik8(args.data_path, args.output_path, args.k, data_percentage=args.data_percentage)
