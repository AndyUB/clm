...
def train_evaluate(
    ...
    if verbose:
        print("Loading data...")
    char_to_idx, idx_to_char, train_loader, val_loader, test_loader = (
    ...
    if verbose:
        print("Saving model...")
    output_dir = os.path.join(output_path, f"gpt2_{text8_percentage}text8")
    os.makedirs(output_dir, exist_ok=True)
    torch.save(model.state_dict(), f"{output_dir}/model.pth")
    with open(f"{output_dir}/char_to_idx.json", "w") as file:
        json.dump(char_to_idx, file)
    with open(f"{output_dir}/idx_to_char.json", "w") as file:
        json.dump(idx_to_char, file)
